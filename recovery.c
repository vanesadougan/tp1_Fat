...
// Adress first FAT = (LBA + reserved sectors) * bytes per sector 
// Addres Root Dir = Address First FAT + NumberFats * Sectores por FAT
// Adress data = Adress root dir + Max number root dir * 32  

//Inicio del siguiente archivo

int address_root_dir;  
int cluster_size;
...
int main() {
    ...
    //Hasta aca el codigo el mismo, ahora tengo que hacer las cuentas especificadas al inicio del archivo, para poder moverme en la tabla FAT y recuperar el archivo.

    cluster_size = bs.bytes_per_sector * bs.clusterSectors;
